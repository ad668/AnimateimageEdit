...
class AdjustFrame(Toplevel):
    ...
    def show_button_release(self, event):
        ...
        img[:, :, 0] = self.gamma_function(img[:, :, 0], 1-warmth)
        img[:, :, 2] = self.gamma_function(img[:, :, 2], 1+warmth)
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        hsv[:, :, 1] = self.gamma_function(hsv[:, :, 1], 1+warmth-0.01)
        ...
        img1[:, :, 0] = self.gamma_function(img1[:, :, 0], 1 + cool)
        img1[:, :, 2] = self.gamma_function(img1[:, :, 2], 1 - cool)
        hsv1 = cv2.cvtColor(img1, cv2.COLOR_BGR2HSV)
        hsv1[:, :, 1] = self.gamma_function(hsv1[:, :, 1], 1 - cool+0.01)
